...
$steps->Given('/^"([^"]*)" としてログインしている$/', function($world, $arg1) {
    // ログイン未実装なので現時点では空にする (1)
});

$steps->Given('/^記事が (\d+) 件登録されている$/', function($world, $arg1) {
    Fabricate::create('post', $arg1, function($data, $world) {
        return ['title' => $world->sequence('titke', function($i){ return "タイトル{$i}"; })];
    }); // (2)
});
...
$steps->Given('/^ページ (\d+) に投稿が新しい順で (\d+) 件表示されている$/', function($world, $page, $count) {
    ...
    $expect = array_chunk(array_map(function($i) {
                return "タイトル{$i}";
            }, range($world->getModel('Post')->find('count'),1)
        ), 5
     )[$page-1];
});
